...
export default class HelpMenuComponent extends Component {
  ...
  render() {
    const embed = new MessageEmbed()
      ...
      .setDescription(
        `:wave: Hello, I'm ${this.props.name}! I'm a Discord bot that runs commands. You can run commands through the chat using the prefix \`${this.props.prefix}\`, or through slash commands.`
      );
      ...
      components: [
        new MessageActionRow().addComponents(
          new MessageButton({
            label: 'Commands',
            style: 'PRIMARY',
            customId: commandsRedirect
          }),
          new MessageButton({
            label: 'Invite',
            style: 'LINK',
            url: this.props.invite
          }),
          new MessageButton({
            label: 'Support Server',
            style: 'LINK',
            url: this.props.supportServer
          })
        )
      ]
    };
